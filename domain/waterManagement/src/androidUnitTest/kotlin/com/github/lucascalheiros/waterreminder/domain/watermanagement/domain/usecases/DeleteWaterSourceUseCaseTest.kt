...
class DeleteWaterSourceUseCaseTest : KoinTest {
    ...
    private lateinit var deleteWaterSourceUseCase: DeleteWaterSourceUseCase

    @Before
    fun setup() {
        deleteWaterSourceUseCase = get()
    }
    ...
    fun `when delete should call repository`() = runTest(testDispatcher) {
        ...
        deleteWaterSourceUseCase.invoke(id)

        coVerify(exactly = 1) {
            get<WaterSourceRepository>().deleteById(id)
        }
    }
    ...
    fun `when repository fails should propagate exception`() = runTest(testDispatcher) {
        ...
        coEvery { get<WaterSourceRepository>().deleteById(id)} throws Exception()
