...
class DeleteConsumedWaterUseCaseTest : KoinTest {
    ...
    private lateinit var deleteConsumedWaterUseCase: DeleteConsumedWaterUseCase

    @Before
    fun setup() {
        deleteConsumedWaterUseCase = get()
    }
    ...
    fun `when delete should call repository`() = runTest(testDispatcher) {
        ...
        deleteConsumedWaterUseCase.invoke(id)

        coVerify(exactly = 1) {
            get<ConsumedWaterRepository>().deleteById(id)
        }
    }
    ...
    fun `when repository fails should propagate exception`() = runTest(testDispatcher) {
        ...
        coEvery { get<ConsumedWaterRepository>().deleteById(id)} throws Exception()
